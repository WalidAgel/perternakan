            ...
            <form action="{{ route('admin.user.update', $user->id) }}" method="POST" class="space-y-6">
                ...
                <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
                    <div class="flex items-start">
                        ...
                        <div>
                            <p class="text-sm font-medium text-blue-800">Informasi Password</p>
                            <p class="text-sm text-blue-700 mt-1">Kosongkan jika tidak ingin mengubah password</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                        Konfirmasi Password Baru <span class="text-gray-400">(Opsional)</span>
                    </label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer hover:border-purple-500 transition
                            {{ old('role', $user->role) === 'admin' ? 'border-purple-500 bg-purple-50' : 'border-gray-300' }}
                        ...
                        <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer hover:border-blue-500 transition
                            {{ old('role', $user->role) === 'karyawan' ? 'border-blue-500 bg-blue-50' : 'border-gray-300' }}
                            @error('role') border-red-500 @enderror">
                            <input type="radio" name="role" value="karyawan"
                                {{ old('role', $user->role) === 'karyawan' ? 'checked' : '' }}
